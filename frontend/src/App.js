...
// not: 721
  ...
  handleBalanceOfAddressChange = (event) => {
    this.setState({ balanceOfAddress: event.target.value });
  };
  
  handleApprovedTokenIdChange = (event) => {
    this.setState({ approvedTokenId: event.target.value });
  };
  
  handleOwnerForAllAddressChange = (event) => {
    this.setState({ ownerForAllAddress: event.target.value });
  };
  
  handleOperatorAddressChange = (event) => {
    this.setState({ operatorAddress: event.target.value });
  };
  ...
  getBalanceOf = async () => {
    const { balanceOfAddress, buyContract } = this.state;
    try {
      const balance = await buyContract.methods.balanceOf(balanceOfAddress).call();
      this.setState({ balanceOfResult: balance });
    } catch (error) {
      console.error('Error fetching balance:', error);
    }
  };
  
  getApprovedAddress = async () => {
    const { approvedTokenId, buyContract } = this.state;
    try {
      const tokenId = parseInt(approvedTokenId);
      const address = await buyContract.methods.getApproved(tokenId).call();
      this.setState({ approvedAddress: address });
    } catch (error) {
  ...
  checkIsApprovedForAll = async () => {
    const { ownerForAllAddress, operatorAddress, buyContract } = this.state;
    try {
      const isApproved = await buyContract.methods.isApprovedForAll(ownerForAllAddress, operatorAddress).call();
      this.setState({ isApprovedForAllResult: isApproved });
    } catch (error) {
    ...
    <Carousel.Caption>
      ...
      <Card className="p-3 bg-white">
        ...
        {/* <Form.Group className="mb-3">
          ...
          <Form.Control type="text" placeholder="Enter owner address" value={this.state.balanceOfAddress} onChange={this.handleBalanceOfAddressChange} />
          <Button variant="primary" onClick={this.getBalanceOf}>Get Balance</Button>
          <p>Balance: {this.state.balanceOfResult}</p>
        </Form.Group> */}

        {/* getApproved UI */}
        {/* <Form.Group className="mb-3">
          <Form.Label>Token ID</Form.Label>
          <Form.Control type="text" placeholder="Enter token ID" value={this.state.approvedTokenId} onChange={this.handleApprovedTokenIdChange} />
          <Button variant="primary" onClick={this.getApprovedAddress}>Get Approved Address</Button>
          <p>Approved Address: {this.state.approvedAddress}</p>
        </Form.Group> */}

        {/* isApprovedForAll UI */}
        {/* <Form.Group className="mb-3">
          <Form.Label>Owner Address</Form.Label>
          <Form.Control type="text" placeholder="Enter owner address" value={this.state.ownerForAllAddress} onChange={this.handleOwnerForAllAddressChange} />
          <Form.Label>Operator Address</Form.Label>
          <Form.Control type="text" placeholder="Enter operator address" value={this.state.operatorAddress} onChange={this.handleOperatorAddressChange} />
          <Button variant="primary" onClick={this.checkIsApprovedForAll}>Check isApprovedForAll</Button>
          <p>isApprovedForAll: {this.state.isApprovedForAllResult.toString()}</p>
        </Form.Group> */}
